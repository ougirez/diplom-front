...
const ChartComponent = ({ selectedCategories, categories }) => {
    useEffect(() => {
        const dataSeries = Object.entries(selectedCategories).flatMap(([group, categoryNames]) => {
            return categoryNames.map(catName => {
                ...
                return {
                    type: "line",
                    name: catName + ` (${category.Unit})`, // Добавляем единицу измерения к имени категории
                    showInLegend: true,
        ...
        const unit = firstSelectedCategory ? categories[Object.keys(categories)[0]].find(cat => cat.Name === firstSelectedCategory).Unit : '';
    ...
    return <div id="chartContainer" className="chart-container"></div>;
};
